...
public abstract class FileSystem extends Configured implements Closeable {
  ...
  public FSDataOutputStream create(Path f, long fileSize) throws IOException {
	    return create(f, fileSize, true);
	  }
  /**
   * Opens an FSDataOutputStream at the indicated Path.
   */
  ...
  public FSDataOutputStream create(Path f, long fileSize, boolean overwrite)
  throws IOException {
  return create(f, fileSize, overwrite,
                getConf().getInt("io.file.buffer.size", 4096));
}
  /**
  ...
  public FSDataOutputStream create(Path f, Long fileSize, Progressable progress) throws IOException {
	    return create(f, fileSize, true,
	                  getConf().getInt("io.file.buffer.size", 4096), progress);
	  }
  /**
   * Opens an FSDataOutputStream at the indicated Path.
   * Files are overwritten by default.
  ...
  public FSDataOutputStream create(Path f, long fileSize,
          boolean overwrite,
          int bufferSize
          ) throws IOException {
	  return create(f, fileSize, overwrite, bufferSize, null);
  }
  /**
   * Opens an FSDataOutputStream at the indicated Path with write-progress
   * reporting.
  ...
  public FSDataOutputStream create(Path f,
	  ...
	  return create(f, fileSize,FsPermission.getDefault().applyUMask(
		        FsPermission.getUMask(getConf())), overwrite, bufferSize, progress);
  }
