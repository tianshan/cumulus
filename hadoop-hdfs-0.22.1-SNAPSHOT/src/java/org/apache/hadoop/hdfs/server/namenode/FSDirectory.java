...
//    return newNode;
  ...
  INodeFileUnderConstruction addFile(String path,
	  ...
	  UnresolvedLinkException{
	    ...
	    if (path.indexOf("/XOR") != -1) {
			//XOR code
	    	type = CodingMatrix.XOR;
		 }
	    else if (path.indexOf("/RC") != -1) {
			//RC code
	    	type = CodingMatrix.RC;
		}else {
			//RS code
			type = CodingMatrix.RS;
		}
    ...
    try {
      ...
      try {
        if (dstInode != null) { // dst exists remove it
          removedDst = removeChild(dstInodes, dstInodes.length - 1);  //tony ,here
